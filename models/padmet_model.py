...
class Padmet(nn.Module):

    def __init__(self, n_targets, num_classes):
        super(Padmet, self).__init__()
...
class MultiPropertyDeepLC(nn.Module):
...
if __name__ == "__main__":
    model = Padmet(n_targets=1, num_classes=11)
    batch_size = 4

    x1 = torch.randn(batch_size, 7).to(torch.float32)
    x2 = torch.randn(batch_size, 50, 6).to(torch.float32)
    x3 = torch.randn(batch_size, 25, 6).to(torch.float32)
    x4 = torch.randn(batch_size, 50, 256).to(torch.float32)
    ...
    torch.onnx.export(
        model,
        (one_hot1, one_hot2, [x1, x2, x3, x4, x5]),
        "padmet2.onnx",
        input_names=['species', 'route', 'channel_1', 'channel_2', 'channel_3', 'channel_4', 'channel_5'],
        output_names=['regression_out', 'classification_out'],
        opset_version=11
    )
    # admet = MultiPropertyDeepLC(model, 1, 15)
